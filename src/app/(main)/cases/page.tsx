                ...
                {loading ? (
                    ...
                    <div className="flex items-center justify-center py-12">
                        <div className="text-center border border-dashed border-slate-300 rounded-2xl px-6 py-10 sm:px-10 bg-white max-w-sm w-full">
                            ...
                            <p className="text-slate-400 text-sm mt-1">Check back soon for new cases.</p>
                        </div>
                    </div>
                ) : (
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
                ...
                <span className="inline-flex items-center gap-1.5 text-xs text-slate-400 shrink-0 mt-1">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {c.files.filter(Boolean).map((f, i) => (
                        <a key={i} href={f} target="_blank" rel="noreferrer"
                            className={`inline-flex items-center gap-1.5 text-xs font-medium border px-3 py-2 sm:py-1.5 rounded-lg transition-colors ${
                                isPptUrl(f)
